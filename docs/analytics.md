...
- **Page views are scrubbed** (`scrubEvent` as `before_send`): `$pageview` fires on history changes so client-side navigation is visible, but every property whose name ends in `url`/`pathname`/`referrer` (including the SDK's `$session_entry_*` and `$initial_*` copies — matched by name so new ones are scrubbed by default) loses its query string and hash, `/durango/<diet>` becomes `/durango/:diet`, `/u/<handle>` becomes `/u/:handle`, and an outside referrer keeps only its origin.
