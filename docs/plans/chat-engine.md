...
| C-D8 | A tool may halt the turn, and `ask_questions` is the first that does | A model that is unsure can always *write* the question and wait — and what comes back is a string ("the first one", "taqueria", "yes") that it then has to interpret, with every later call resting on that reading. `ask_questions` has the server write the options down, the client render exactly those, and the answer return as an **option id**. An id not on the stored list is refused rather than accepted as free text, which is the property the whole tool exists for; a free-text escape hatch stays, marked as the person's own words, because an option list that misses the obvious answer is worse than no options. It halts because a question the model has already answered around is not a question. `halts_turn` is deliberately a general mechanism and deliberately *not* part of `ModePolicy` — halting is not a permission decision and applies in every mode. Parking happens on the decision to call, before dispatch, exactly like a confirmation and for the same reason: this call's `tool_result` has to be the person's answer, so the tool must not already have written one. A malformed question comes back as a recoverable tool error rather than parking, because a park nobody can clear needs a human with database access. |
