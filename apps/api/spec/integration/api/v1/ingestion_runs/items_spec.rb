    ...
    patch("Decide a staged item: accept / edit / reject / undo") do
      ...
      parameter name: :body, in: :body, required: true, schema: {
        ...
        description: "decision: accepted promotes (admin ⇒ confirmed joins, community ⇒ " \
                     ...
                     "complete array, omit the key to leave it alone, send [] to clear " \
                     "the staged row. Prices on a MATCHED card are the exception: an " \
                     "empty array clears the staged payload but leaves the live item's " \
                     "variants alone (an empty scan means 'no prices seen').",
        properties: {
          ...
          prices_payload: {
            ...
            description: "Materialized as ItemVariants at promote, in array order. " \
                         "Rows without price_cents are dropped; price_cents must be a " \
                         "non-negative integer (422 otherwise).",
            items: {
              type: :object,
              properties: {
